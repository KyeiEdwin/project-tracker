...
<template>
  <aside class="workspace-sidebar" :class="{ 'is-open': open }">
    <router-link to="/" class="brand" @click="emit('navigate')"><span class="brand-mark"><i class="ri-focus-3-line"></i></span><span>Northstar<small>PROJECTS</small></span></router-link>
    <nav class="sidebar-nav" aria-label="Main navigation">
      <template v-for="item in menuItems" :key="item.id">
        <button v-if="item.children" class="nav-item nav-toggle" :class="{ active: isChildActive(item.children) }" @click="toggle(item)"><i :class="item.icon"></i><span>{{ item.label }}</span><i class="ri-arrow-down-s-line nav-chevron" :class="{ rotated: menuOpen(item) }"></i></button>
        <div v-if="item.children && menuOpen(item)" class="nav-children"><router-link v-for="child in item.children" :key="child.to" :to="child.to" :class="{ active: isActive(child.to) }" @click="emit('navigate')">{{ child.label }}</router-link></div>
        <router-link v-else :to="item.to" class="nav-item" :class="{ active: isActive(item.to) }" @click="emit('navigate')"><i :class="item.icon"></i><span>{{ item.label }}</span></router-link>
      </template>
    </nav>
